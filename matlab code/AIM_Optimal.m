...
while(frameCounter<duration*fps) 
    ...
    for k=1:4
        ...
        if(k==1 || k==3)
            ...
            elseif(turnRand>=0.5 && turnRand<0.75)
            ...
            if(turnRand<0.30)
                turnDecision = turns{1};%Go Straight
            elseif(turnRand>=0.3 && turnRand<0.65)
                turnDecision = turns{2};%Turn Left
            else
                turnDecision = turns{3};%Turn Right
            end
...
%             totalVehicles(k) = totalVehicles(k) + platoonSize;
%             spawnp = intersectionData.spawnPoints(k,:)+offset(k,:);
%             platoonLeader = vehicleModel(spawnp);
