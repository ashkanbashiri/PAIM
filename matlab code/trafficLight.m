function [fuelConsumptionPerVehicle,F,callCounter,packets,var,AverageDelayPerVehicle,AverageDelayPerPlatoon,totalVehicles,totalVehiclesCrossed] = trafficLight(policyName,ver,seed,granularity,platoonMaxSize,spawnRate,duration,simSpeed,handles)
tic;
makeVideo=0;
if(makeVideo==0)
    F=[];
...
%intersectionData.title = '6-Phase Traffic Light';
map = intersectionData.getMap(granularity,'6-Phase Traffic Light');
show(map)
...
while(frameCounter<duration*fps)
    framesPassed = frameCounter-phaseTurnFrame;
    if(rem(framesPassed,phaseTimes(currentPhase)/simulationTime)==0)
        phaseChange=1;
        phaseTurnFrame = frameCounter;
        if(yellowLight==0)
            yellowLight=1;
            lastPhase = currentPhase;
            currentPhase = 7;%yellow light
        else
        ...
        for j=sortedList
            ...
            if(isvalid(platoons(j)) )
                ...
                if(timePassed>=clearTime && j==whosTurn ...
                    ...
                    clearTime = arrival + (25+(platoons(j).platoonSize*6))/(15*simulationTime);
                    lastFrame = frameCounter;
...
var = sum(delays.^2)/(length(delays)-1) - (length(delays))*mean(delays)^2/(length(delays)-1);
    ...
    function [newSchedule] = tlSchedule(candidates)
        newSchedule = [];
        stillLooking=1;
        leftout = [];
        for(i=1:length(candidates))
            turnNumber = getTurnNumber(platoons(candidates(i)).arrivalLane, platoons(candidates(i)).turn);
            if(stillLooking)
                if(~isempty(find(phaseTurns(currentPhase,:)==turnNumber)))
                    newSchedule(1) = candidates(i);
                    stillLooking = 0;
        ...
        cops = [];
        for my_index=1:length(leftout)
            turnNumber = getTurnNumber(platoons(leftout(my_index)).arrivalLane, platoons(leftout(my_index)).turn);
            if(~isempty(find(phaseTurns(currentPhase,:)==turnNumber)))
                cops = [cops leftout(my_index)];
