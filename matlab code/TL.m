...
show(map)
rectangle('Position',[0 0 190 190],'FaceColor',[0 .5 .5],...
    'LineWidth',2);
rectangle('Position',[210 0 400 190],'FaceColor',[0 .5 .5],...
    'LineWidth',2);
rectangle('Position',[0 210 190 400],'FaceColor',[0 .5 .5],...
    'LineWidth',2);
rectangle('Position',[210 210 400 400],'FaceColor',[0 .5 .5],...
    'LineWidth',2);
            ...
            for pt=sortedList
                if(candidateCounter==4)
                    ...
                    if(candidates(lane)==0 && (strcmp(platoons(pt).state,'stopandwait')||...
                            ...
                            && (platoons(pt).arrivalTime)<=40 && platoons(pt).reachedCollisionZone==1)
                        candidates(lane)=pt;
                        platoons(pt).color = 'y';
        ...
        for j=sortedList
            ...
            if(isvalid(platoons(j)) )
                ...
                if(timePassed>=clearTime && j==whosTurn ...
                    ...
                    clearTime = arrival + (25+(platoons(j).platoonSize*10))/(7*simulationTime);
                    lastFrame = frameCounter;
                    ...
                    for coCounter=1:length(platoons(j).coPlatoon)
                        if(platoons(j).coPlatoon(coCounter)~=-1 && sum(find(coplatoons==platoons(j).coPlatoon(coCounter)))==0 ...
                            ...
                            clearTime = max(clearTime, arrival + (25+(platoons(temp).platoonSize*10))/(7*simulationTime));
                            lastFrame = frameCounter;
